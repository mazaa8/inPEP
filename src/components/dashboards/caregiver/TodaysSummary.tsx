...
const TodaysSummary = ({ mood, notes, tasks }: TodaysSummaryProps) => {
  return (
    <Card sx={{ background: 'linear-gradient(135deg, #F4F7F6 0%, #E9EFEE 100%)', color: '#2D4A43', borderRadius: '16px', boxShadow: '0 12px 32px rgba(0,0,0,0.1)' }}>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, pl: 1 }}>
          ...
          <Typography variant="body1">Feeling: {mood}</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', pl: 1 }}>
          <Notes sx={{ mr: 1.5, color: '#4A7C6E' }} />
          <Typography variant="body1">Notes: {notes}</Typography>
        </Box>
        <Box sx={{ mt: 2, pl: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
